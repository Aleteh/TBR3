...
-- TODO: Make it dynamic (probably will need to change the method for providing bonus power, 'AdjustSpellPower' that calculates it from 0)
function PrometheusPower( event )
	local hero = event.caster
	local spellPower = hero.spellPower
	local healingPower = hero.healingPower
	local powerMultiplier = event.ability:GetSpecialValueFor("bonus_power_percent") * 0.01
	local BonusSpellPower = spellPower * powerMultiplier
	local BonusHealingPower = spellPower * powerMultiplier

	-- Store to remove when dropped
	hero.PrometheusSpellPower = BonusSpellPower
	hero.PrometheusHealingPower = BonusHealingPower

	-- Add the bonus
	print("+" .. BonusSpellPower .. " Spell Power from Prometheus")
	hero.spellPower = hero.spellPower + BonusSpellPower
	print("Current Spell Power: " .. hero.spellPower)
	
	print("+" .. BonusHealingPower .. " Healing Power from Prometheus")
	hero.healingPower = hero.healingPower + BonusHealingPower
	print("Current Healing Power: " .. hero.healingPower)

end

function DestroyPrometheusPower( event )
	local hero = event.caster
	local spellPower = hero.spellPower
	local healingPower = hero.healingPower

	-- Remove the bonus
	hero.spellPower = spellPower - hero.PrometheusSpellPower
	hero.healingPower = healingPower - hero.PrometheusHealingPower

	print("-" .. hero.PrometheusSpellPower .. " Spell Power")
	print("Current Spell Power: " .. hero.spellPower)
	print("-" .. hero.PrometheusHealingPower .. " Healing Power")
	print("Current Healing Power: " .. hero.healingPower)
end